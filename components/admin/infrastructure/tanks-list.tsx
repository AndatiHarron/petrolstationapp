...
export function TankCardSkeleton() {
    return (
        <View className="bg-slate-800 border border-slate-700/50 rounded-xl p-4 mb-3">
            ...
            <View className="h-2 w-full bg-slate-700 rounded-full mb-2 animate-pulse" />
            ...
            <View className="flex-row items-start justify-between mb-3">
                <View className="flex-1">
                    <View className="flex-row items-center">
                        <Cylinder size={18} color="#94a3b8" />
                        <Text className="text-white font-semibold text-base ml-2">{tank.name}</Text>
                    </View>
                    {tank.product_name ? (
                        <View className="flex-row items-center mt-1">
                            <Droplet size={12} color="#3b82f6" />
                            <Text className="text-blue-400 text-sm ml-1">{tank.product_name}</Text>
                        </View>
                    ) : null}
                    {tank.station_name ? (
                        <View className="flex-row items-center mt-1">
                            <MapPin size={12} color="#64748b" />
                            <Text className="text-slate-500 text-xs ml-1">{tank.station_name}</Text>
                        </View>
                    ) : null}
                </View>
        ...
        <View className="flex-1">
            ...
            <View className="flex-row items-center justify-between mb-4">
                ...
                {onAddTank ? (
                    ...
                    >
                        ...
                        <Text className="text-white font-medium text-sm ml-1">Add Tank</Text>
                    </Pressable>
                ) : null}
            </View>
            ...
            {tanks.length === 0 ? (
                <View className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-8 items-center">
                    <Cylinder size={48} color="#64748b" />
                    <Text className="text-slate-400 text-lg mt-4">No tanks found</Text>
                    <Text className="text-slate-500 text-sm mt-1">Add a tank to get started</Text>
                </View>
            ) : (
                <FlashList
                    data={tanks}
