...
const getStatusColor = (status: string) => {
    switch (status) {
        ...
        case 'rejected': return { bg: 'bg-accent/15', text: 'text-accent', border: 'border-accent/20' };
...
const RequestItem = memo(({ item, onPress }: { item: EditRequestResource; onPress: (item: EditRequestResource) => void }) => {
    ...
    return (
        ...
        >
            ...
            {item.reason && (
                <View className="bg-surface-border p-2.5 rounded-lg mt-1">
                    <Text className="text-gray-600 text-xs uppercase mb-1">Reason</Text>
                    <Text className="text-ink text-sm" numberOfLines={2}>{item.reason}</Text>
                </View>
            )}
        </TouchableOpacity>
    );
    ...
    return (
        <View className="flex-row justify-between items-center py-2 border-b border-surface-border">
            ...
            <View className="flex-row items-center gap-2">
                <Text className={`text-sm font-mono ${changed ? 'text-accent line-through' : 'text-ink'}`}>
                    {String(original ?? '—')}
                </Text>
                {changed && (
                    <>
        ...
        <View className="flex-1 bg-surface-sunken">
            <SafeAreaView className="flex-1" edges={['top']}>
                ...
                <Animated.View entering={FadeInDown.duration(400)} className="px-4 pt-4 pb-2">
                    <Text className="text-ink text-2xl font-bold">Edit Requests</Text>
                    ...
                    <FlatList<EditRequestResource>
                        ...
                        ListEmptyComponent={() => (
                            <View className="items-center justify-center py-20">
                                <Ionicons name="document-text-outline" size={56} color="#5c5c6b" />
                                <Text className="text-ink-muted text-center mt-4 text-base">
            ...
            >
                <View className="flex-1 justify-end">
                    <View className="bg-surface-sunken border-t border-surface-border h-[85%] rounded-t-3xl shadow-2xl">
                        {/* Modal Header */}
                        <View className="p-5 border-b border-surface-border flex-row justify-between items-center bg-surface-sunken rounded-t-3xl">
                                    ...
                                    <>
                                        ...
                                        <View className="bg-surface p-4 rounded-xl mb-4">
                                            <View className="flex-row justify-between mb-2 pb-2 border-b border-surface-border">
                                                <Text className="text-ink-muted text-sm">Requested by</Text>
                                                <Text className="text-ink font-medium">{selectedRequest.user?.name ?? 'Unknown'}</Text>
                                            </View>
                                            <View className="flex-row justify-between mb-2 pb-2 border-b border-surface-border">
                                                <Text className="text-ink-muted text-sm">Model</Text>
                                                <Text className="text-ink font-medium">{getModelLabel(selectedRequest.model_type)}</Text>
                                            </View>
                                            <View className="flex-row justify-between">
                                                <Text className="text-ink-muted text-sm">Submitted</Text>
                                                <Text className="text-ink font-medium">{formatDate(selectedRequest.created_at)}</Text>
                                            </View>
                                        </View>
                                        ...
                                        {selectedRequest.reason && (
                                            <View className="bg-amber-500/10 border border-amber-500/20 p-4 rounded-xl mb-4">
                                                ...
                                                <Text className="text-ink text-sm leading-5">{selectedRequest.reason}</Text>
                                            </View>
                                        )}

                                        {/* Data Diff */}
                                        <View className="bg-surface p-4 rounded-xl mb-4">
                                            <Text className="text-ink-muted text-xs uppercase font-bold mb-3">Changes</Text>
                                            {originalData && requestedData ? (
                                                <DataDiff label="Data" original={originalData} requested={requestedData} />
                                            ) : (
                                                <View>
                                                    <Text className="text-ink-muted text-xs uppercase mb-1">Original</Text>
                                                    <Text className="text-ink text-sm mb-3 font-mono">{JSON.stringify(selectedRequest.original_data, null, 2) || '—'}</Text>
                                                    <Text className="text-ink-muted text-xs uppercase mb-1">Requested</Text>
                                        ...
                                        {selectedRequest.approver && (
                                            <View className="bg-surface p-4 rounded-xl mb-4">
                                                <Text className="text-ink-muted text-xs uppercase font-bold mb-2">Reviewed By</Text>
                                                <Text className="text-ink font-medium">{selectedRequest.approver.name}</Text>
                                                {selectedRequest.comments && (
                                                    <Text className="text-ink text-sm mt-2">{selectedRequest.comments}</Text>
                                                )}
                                            </View>
                                        )}
